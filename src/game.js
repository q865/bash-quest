...
class Game {
    constructor() {
        ...
        this.levels = this.levelManifest.blocks.flatMap(block => block.levels);
    ...
    async loadValidators() {
        const validatorFiles = fs.readdirSync(VALIDATORS_DIR).filter(file => file.endsWith('.js'));
        for (const file of validatorFiles) {
            const validatorName = path.basename(file, '.js');
            const validatorPath = path.join(VALIDATORS_DIR, file);
            const { default: validatorFunc } = await import(pathToFileURL(validatorPath).href);
    ...
    async loadActions() {
        ...
        for (const file of actionFiles) {
            ...
            const { default: actionFunc } = await import(pathToFileURL(actionPath).href);
    ...
    loadLevelData(levelId) {
        ...
        const levelFile = path.resolve(__dirname, '..', levelInfo.path, 'level.json');
    ...
    getHint() {
        const levelData = this.loadLevelData(this.state.currentLevel);
        return levelData?.hint || 'Подсказок больше нет.';
    }

    async checkSolution() {
        const levelData = this.loadLevelData(this.state.currentLevel);
        if (!levelData || !levelData.validation) {
            return { success: false, message: 'Не удалось загрузить проверку для этого уровня.' };
        }

        const validations = Array.isArray(levelData.validation) ? levelData.validation : [levelData.validation];
        ...
        for (const validation of validations) {
            ...
            if (!result.success) {
                return { success: false, message: result.message || 'Решение неверное.' };
            }
        }
        ...
        const currentLevelIndex = this.levels.findIndex(l => l.id === this.state.currentLevel);
    ...
    reset() {
        this.state = { currentLevel: this.levels[0].id, completedLevels: [] };
        this.saveState();
