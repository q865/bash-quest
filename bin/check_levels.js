...
function checkLevels() {
    ...
    try {
        ...
        console.error(
            `❌ Ошибка чтения или парсинга файла ${levelsFile}:`,
            error
        );
        process.exit(1);
    }
    ...
    if (!blocks || !Array.isArray(blocks)) {
        console.error(
            `❌ Файл ${levelsFile} должен содержать массив 'blocks'.`
        );
        process.exit(1);
    }
...
🔎 Проверяю блок: "${block.title}"`);
        if (!block.levels || !Array.isArray(block.levels)) {
            console.error(
                `  ❌ У блока "${block.title}" отсутствует или некорректен массив 'levels'.`
            );
            errorCount++;
            continue;
        }
        ...
        for (const level of block.levels) {
            if (!level.id || !level.title || !level.path) {
                console.error(
                    `  ❌ Некорректная запись уровня: ${JSON.stringify(level)}`
                );
                errorCount++;
                continue;
            }
            ...
            if (!fs.existsSync(levelPath)) {
                console.error(
                    `  ❌ Директория для уровня ${level.id} ("${level.title}") не найдена по пути: ${levelPath}`
                );
                errorCount++;
                continue;
            }
            ...
            if (!fs.existsSync(levelJsonPath)) {
                console.error(
                    `  ❌ Файл level.json для уровня ${level.id} не найден в: ${levelPath}`
                );
                errorCount++;
            }
            ...
            if (!fs.existsSync(taskMdPath)) {
                console.error(
                    `  ❌ Файл task.md для уровня ${level.id} не найден в: ${levelPath}`
                );
                errorCount++;
            }
            ...
            if (errorCount === 0) {
                console.log(
                    `  ✅ Уровень ${level.id} ("${level.title}") в порядке.`
                );
            }
        }
    }
